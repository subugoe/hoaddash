...
The share of Open Access articles with a Creative Commons license in hybrid journals exposed by the [German Open Access Monitor from the FZ Jülich](https://open-access-monitor.de/) is as follows:
...
```{r}
cc_jn_ind_pub <- hoaddata::oam_hybrid_jns |>
  select(jn_id = cr_journal_id, agreement,lead) |>
...
de_df <- hoaddata::oam_hybrid_jns |>
  mutate(esac_publisher = paste0(agreement, " (", lead, ")")) |>
  distinct(jn_id = cr_journal_id, esac_publisher) |>
  mutate(publisher_fct = forcats::fct_other(esac_publisher, keep = top)) |>
  mutate(publisher_fct = forcats::fct_relevel(publisher_fct, top, "Other")) |>
...
pub_shares <- bind_rows(all_publisher_league, cc_publisher_league) |>
  mutate(type = case_when(
    publisher_fct == "Springer Hybrid (DEAL)" ~ "Springer Hybrid (DEAL)",
    publisher_fct == "Wiley Hybrid (DEAL)" ~ "Wiley Hybrid (DEAL)",
    publisher_fct %in% top_publisher[3:13] ~ "Top 4-13",
...
::: column-body-outset
{{< include _oam_publisher_league.qmd >}}
:::
...
::: column-body-outset
{{< include _oam_country_top20.qmd >}}
:::
...
{{< include _oam_country_league_table.qmd >}}
...
{{< include _oam_open_md_coverage.qmd >}}
